...
public class Recommender
{
	public static void main(String[] args)
	{
		...
		db.query("SELECT DISTINCT title, year, rtAudienceScore, rtPictureURL, imdbPictureURL FROM movies ORDER BY rtAudienceScore DESC LIMIT " + Integer.parseInt(s));
		
		System.out.println("\nType a movie to see pictures for that movie.");
		s = scanner.nextLine();
		
		db.query("SELECT DISTINCT title, m.year, m.rtAudienceScore, m.rtPictureURL, m.imdbPictureURL, t.value FROM movies m, tags t, user_taggedmovies_timestamps u WHERE m.title LIKE \'%" + s + "%\' AND m.id = u.movieID AND u.tagID = t.id ORDER BY title");
		...
		db.query("SELECT DISTINCT t.value FROM tags t, movie_tags mt, movies m WHERE mt.tagID = t.id AND m.title LIKE \'%" + s + "%\' AND m.id = mt.movieID");
	}
}
